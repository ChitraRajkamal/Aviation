...
@section('title', 'Buy - ' . $recordedVideo->title)
...
<style>
    ...
    .course-prepare-info-each > span:last-child{
        color: black;
    }
    #make-payment{
        font-size: 30px;
            ...
            <div class="col-lg-12">
                <div class="single-course-left-align-wrapper">
                    <div class="meta-area">
                        ...
                        <i class="fa-solid fa-chevron-right"></i>
                        <a href="{{lms_recorded_video_slug($recordedVideo)}}">{{$recordedVideo->title}}</a>
                        <i class="fa-solid fa-chevron-right"></i>
                    ...
                    <div class="author-area mt-1">
                        ...
                        <p class="mb-0"> <span>Category: </span> {{$recordedVideo->category->title}}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row g-5">
            <div class="col-lg-8">
                <div class="text-center">
                    <div class="course-prepare-info-each">
                        <label>Amount</label>
                        <span class="fa-2x">{{$recordedVideo->price}}</span>
                    </div>
...
<script>
    var options = {
        ...
        "description": "{{lms_setting('app_name')}} - Exam Payment - {{$recordedVideo->title}}",
                ...
                setTimeout(() => {
                    ...
                    $.post('{{route('ajax.verify_razorpay_payment')}}', {
                        ...
                        if (data.status == 'error') {
                            ...
                            $('#make-payment').prop('disabled', true).html('<i class="fa fa-spin fa-circle-notch"></i> Redirecting to course page...');
                            notify({ title: data.message, type: 'success' });
                            setTimeout(() => {
                                show_loader();
                                window.location.href = "{{lms_recorded_video_slug($recordedVideo)}}";
                            }, 3000);
                        }
                    }, 'JSON');
                }, 100);
