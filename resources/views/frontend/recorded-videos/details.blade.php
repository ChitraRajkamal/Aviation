...
@section('title', $recordedVideo->title)
@php
    $meta_data = $recordedVideo->meta_data ?? [];
    $sections = [];
    $total_lessons = 0;
    $total_quizs = 0;
    $requirements = false;
...
@section('meta-data')
    ...
    <meta name="twitter:title" content="{{ $meta_data['og_title'] ?? $meta_data['title'] ?? $recordedVideo->title }}">
            ...
            <div class="col-lg-12">
                <div class="single-course-left-align-wrapper">
                    <div class="meta-area">
                        ...
                        <a href="{{route('recorded-videos')}}">Videos</a>
                        <i class="fa-solid fa-chevron-right"></i>
                        <a class="active" href="#">{{$recordedVideo->title}}</a>
                    </div>
                    <h1 class="title">
                        {{$recordedVideo->title}}
                    </h1>
                    <div class="rating-area">
                        ...
                        <div class="students">
                            ...
                            <span>{{$recordedVideo->enroll_count()}} Enrolled</span>
                        </div>
                        <div class="calender-area-stars">
                            <i class="fa-light fa-calendar-lines-pen"></i>
                            <span>Last updated {{lms_format_date($recordedVideo->updated_at)}}</span>
                        </div>
                    </div>
                    <div class="author-area">
                        <div class="author">
                            <img src="{{asset('admin-assets/assets/images/profile-40.png')}}" alt="Instructor">
                            <div class="name fw-medium"><span>By</span> {{$recordedVideo->user->first_name}}.</div>
                        </div>
                        <p> <span>Category: </span> {{$recordedVideo->category->title}}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-8 order-cl-1 order-lg-1 order-md-2 order-sm-2 order-2">
                @if (!$recordedVideoEnrolled)
                    <div class="border border-2 border-primary text-dark py-5 mb-5 rounded shadow text-center">
                        ...
                        Please enroll to view video details and view the recorded video.
                    </div>
                @endif
                ...
                <div class="tab-content mt--20" id="myTabContent">
                    <div>
                        @if (lms_user_id() && $recordedVideoEnrolled)
                            @if ($rated)
                                <div class="alert alert-info mb-4"><i class="fa fa-check-circle"></i> {{ __('You already rated this recorded video') }}</div>
                            @else
                                @error('type')
                    ...
                    <div class="course-side-bar">
                        @if ($recordedVideoEnrolled)
                            <div class="thumbnail">
                                ...
                                <div class="vedio-icone">
                                    <a class="video-play-button play-video popup-video" href="{{$recordedVideo->link}}">
                                        <span></span>
                                    </a>
                        ...
                        <div class="price-area {{$recordedVideo->thumbnail ? 'mt-5' : 'mt-0'}}">                            
                            @if ($recordedVideo->discount_flag)
                                ...
                                <span class="discount">-{{(100 - round(($recordedVideo->discounted_price/$recordedVideo->price) * 100, 2))}}%</span>
                            @else
                        ...
                        <div class="what-includes">
                            <div class="single-include">
                                ...
                                <div class="right">
                                    <span>{{$recordedVideo->category->title}}</span>
                                </div>
                            </div>
                            <div class="single-include">
                                <div class="left">
                                ...
                                <div class="right">
                                    <span>{{lms_format_date($recordedVideo->updated_at)}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="right-course-details mt--30">
                    ...
                    <div class="course-side-bar">
                        ...
                        <div class="course-single-information">
                            <h5 class="title">A video by</h5>
                            <div class="body">
                                <div class="author">
                                    <img src="{{asset('admin-assets/assets/images/profile-40.png')}}" alt="Instructor">
                                    <span>{{$recordedVideo->user->first_name}}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="course-single-information last">
                            ...
                            <div class="body">
                                ...
                                <div class="single-check">
                                    <i class="fa-light fa-circle-check"></i>
                                    Suitable for {{$recordedVideo->level}} students
                                </div>
                                <!-- ingle check end -->
                            </div>
                        </div>
...
<script>
    ...
    const courseTitle = encodeURIComponent("{{$recordedVideo->title}}");
