                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        @if (request('filter'))
                                            <a href="{{ route('admin.recorded-video-categories.index') }}" class="btn btn-default btn-sm m-b-0 bs-tt sloc" title="Reset Filter"><i class="fa fa-undo"></i> Clear</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                ...
                <tbody>
                    @foreach ($categoryList as $item) 
                        ...
                        <tr class="tr_{{$item->id}}">
                            ...
                            <td>
                                <a href="{{route('admin.recorded-videos.index').'?filter=true&category_id='.$item->id}}" class="bs-tt" title="View Recorded Videos" data-placement="top">
                                    <span class="badge badge-primary">{{$item->recorded_video_count()}}</span>
                                </a>
                            </td>
                            <td>{{lms_format_date($item->created_at)}}</td>
                            <td class="dt-actions">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('admin.recorded-video-categories.edit', $item) }}" class="btn btn-primary btn-mini sloc bs-tt mr-2" title="Edit Item" data-placement="left">
                                        <i class="ph ph-pencil-simple mr-0"></i>
                                    </a>
                                    <form action="{{ route('admin.recorded-video-categories.activate', ['id' => $item->id, 'status' => $item->status ? 0 : 1]) }}" id="activate-form-{{$item->id}}" 
                                    ...
                                    <form action="{{ route('admin.recorded-video-categories.destroy', $item) }}" id="delete-form-{{$item->id}}" 
