...
@section('title', __('Recorded Videos'))

@section('page-header')
    @include('layouts.admin.page-header', [
        'icon' => 'ph ph-video text-muted',
        'title' => __('Recorded Videos'),
        'breadcrumbs' => [
            route('admin.recorded-videos.index') => __('Recorded Videos')
        ],
        'links' => [
            lms_is_admin() ? generate_link_element([
                'route' => 'admin.recorded-videos.create'
            ]) : false,
        ]
    ])
@endsection
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        @if (request('filter'))
                                            <a href="{{ route('admin.recorded-videos.index') }}" class="btn btn-default btn-sm m-b-0 bs-tt sloc" title="Reset Filter"><i class="fa fa-undo"></i> Clear</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
    ...
    <div class="card-block">
        <div class="dt-responsive table-responsive dt-fixed-controls">
            <table id="listing-data-table" class="g-datatable table compact table-striped table-hover table-bordered table-header-dark nowrap">
                ...
                <tbody>
                    @foreach ($recordedVideoList as $item)
                        @php
                            ...
                            <td class="dt-actions">
                                <div class="btn-group" role="group">
                                    @if ($item->organization_id == $global_organization_id)
                                        <a href="{{ route('admin.recorded-videos.edit', $item) }}" class="btn btn-primary btn-mini sloc bs-tt mr-2" title="Edit Item" data-placement="left">
                                            <i class="ph ph-pencil-simple mr-0"></i>
                                        </a>
                                        <form action="{{ route('admin.recorded-videos.activate', ['recordedVideoId' => $item->id, 'status' => $item->status ? 0 : 1]) }}" id="activate-form-{{$item->id}}"
                                        ...
                                        <form action="{{ route('admin.recorded-videos.destroy', $item) }}" id="delete-form-{{$item->id}}"
                                    ...
                                    @else
                                        <a href="{{ route('admin.recorded-videos.enrolments', ['recordedVideoId' => $item->id]) }}" class="btn btn-primary btn-mini sloc bs-tt mr-2" title="Edit Item" data-placement="left">
                                            <i class="ph ph-user-plus mr-0"></i> Enrolments
                                        </a>
                                    @endif
                                    <a href="{{ route('admin.recorded-videos.ratings', $item) }}" class="btn btn-yellow btn-mini sloc bs-tt mr-2" title="View Ratings" data-placement="left">
