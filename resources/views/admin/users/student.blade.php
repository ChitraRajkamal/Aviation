...
@section('page-header')
    @include('layouts.admin.page-header', [
        'icon' => 'ph ph-graduation-cap text-muted',
        'title' => __(key: 'Students'),
        'breadcrumbs' => [
            route('admin.users.student') => __('Students')
        ],
        'links' => [
            lms_can_access('admin.users.student.create') ? generate_link_element([
                'route' => 'admin.users.student.create'
            ]) : false,
            lms_can_access('admin.users.student.import') ? generate_link_element([
                'label' => '<i class="ph ph-upload mb-0 mr-0"></i> ' . __('Import'),
                'action' => __('Import'),
                'title' => __('Import Bulk Students'),
                'class' => 'btn py-1 px-2 btn-primary bs-tt',
                'route' => 'admin.users.student.import',
            ]) : false,
        ]
    ])
@endsection
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        @if (request('search'))
                                            <a href="{{ route('admin.users.student') }}" class="btn btn-default btn-sm m-b-0 bs-tt sloc" title="Reset Filter"><i class="fa fa-undo"></i> Clear</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <tr class="tr_{{$item->id}}">
                            ...
                            <td class="dt-actions">
                                <div class="btn-group" role="group">
                                    @if (lms_can_access('admin.users.student.edit'))
                                        <a href="{{ route('admin.users.student.edit', ['id' => $item->id]) }}" class="btn btn-primary btn-mini sloc bs-tt" title="Edit Student" data-placement="left">
                                            <i class="ph ph-pencil-simple mr-0"></i>
                                        </a>
                                    @endif
                                    @if (lms_can_access('admin.users.student.activate'))
                                        <form action="{{ route('admin.users.student.activate', ['userId' => $item->id, 'status' => $item->status ? 0 : 1]) }}" id="activate-form-{{$item->id}}" 
                                            class="d-inline-block mb-0 confirm-action ml-1" data-action="submit" 
                                            data-element_id="activate-form-{{$item->id}}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="button" class="btn btn-{{$item->status ? 'warning' : 'success'}} btn-mini bs-tt" title="{{$item->status ? 'Deactivate' : 'Activate'}} Student" data-placement="left">
