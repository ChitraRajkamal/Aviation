...
@section('content')
    ...
    <form id="saveForm" class="manualSubmission" action="{{ route('admin.roles.organization.save') }}" method="POST" data-parsley-validate>
        @csrf
        <input type="hidden" id="menu_ids" name="menu_ids" value="@Model.menu_ids" />
                ...
                <div class="row">
                    ...
                    <div class="col-md-5" id="action-container">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="role_name" name="name" value="{{ old('name', $role->name)}}" placeholder="{{__('Enter Role Name')}}" @lmsparsley(organization_roles_add,name)>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="description" name="description" class="form-control h-180" rows="4"placeholder="{{__('Enter Description')}}" @lmsparsley(organization_roles_add,description)>{{ old('description', $role->description)}}</textarea>
                            @error('description')
                                <div class="parsley-errors-list">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var Oldmenu_ids = JSON.parse(decode_html_entities('{{$role->menu_ids}}'));
