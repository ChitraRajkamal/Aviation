...
@section('content')
<form id="saveForm" class="autoSubmit" action="{{ route('admin.exams.update', ['exam' => $exam]) }}" method="POST" enctype="multipart/form-data" data-parsley-validate>
    @csrf
    @method('PUT')
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="title" value="{{ old('title', $exam->title)}}" placeholder="{{__('Enter Exam Title')}}" @lmsparsley(exams_add,title)>
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select-two" id="exam_category_id" data-placeholder="Choose Category" name="exam_category_id" @lmsparsley(exams_add,exam_category_id)>
                            ...
                            @foreach ($categoriesHierarchy as $item)
                              <option value="{{$item->id}}" {{ old('exam_category_id', $exam->exam_category_id) == $item->id ? 'selected' : '' }}>{{$item->title}}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        value="{{ old('duration', $exam->duration)}}" placeholder="{{__('01:30:00 ==> One Hour & 30 Minutes')}}" @lmsparsley(exams_add,duration)>
                        @error('duration')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table id="selected-questions" class="table mb-0">
                            ...
                            <tbody>
                                @if (old('qbank_ids', $exam->qbank_ids))
                                    @foreach (old('qbank_ids', $exam->qbank_ids) as $q)
                                        @php
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" min="0" class="form-control" id="total_mark" name="total_mark" value="{{ old('total_mark', $exam->total_mark)}}" readonly
                            placeholder="{{__('Total available marks')}}" @lmsparsley(exams_add,total_mark)>
                        @error('total_mark')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        value="{{ old('pass_mark', $exam->pass_mark)}}" placeholder="{{__('Totals marks needed to pass')}}" @lmsparsley(exams_add,pass_mark)>
                        @error('pass_mark')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="number" min="0" max="100" class="form-control" name="negative_mark" value="{{ old('negative_mark', $exam->negative_mark)}}" 
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        value="{{ old('retake', $exam->retake)}}" placeholder="{{__('How many reattempts are allowed. Set 0 if unlimited')}}" @lmsparsley(exams_add,retake)>
                        @error('retake')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control decimal_input" name="price" value="{{ old('price', $exam->price)}}" placeholder="{{__('Enter Price')}}" @lmsparsley(exams_add,price)>
                        @error('price')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        @if ($exam->thumbnail)
                          <img src="{{lms_storage($exam->thumbnail)}}" class="img-thumbnail mt-3" style="height: 140px;" alt="">
                        @endif
                        @error('thumbnail')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" class="form-control" rows="4"placeholder="{{__('Enter Description')}}" @lmsparsley(exams_add,description)>{{ old('description', $exam->description)}}</textarea>
                        @error('description')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="meta_data[title]" value="{{ old('meta_data.title', $exam->meta_data['title'] ?? '') }}" placeholder="{{__('Enter Meta Title')}}" >
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="meta_data[robot]" value="{{ old('meta_data.robot', $exam->meta_data['robot'] ?? '') }}" placeholder="{{__('Enter Meta Robot')}}" >
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="meta_data[description]" class="form-control" rows="4" placeholder="{{__('Enter Meta Description')}}">{{ old('meta_data.description', $exam->meta_data['description'] ?? '') }}</textarea>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="meta_data[og_title]" value="{{ old('meta_data.og_title', $exam->meta_data['og_title'] ?? '') }}" placeholder="{{__('Enter Og Title')}}" >
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="meta_data[og_description]" value="{{ old('meta_data.og_description', $exam->meta_data['og_description'] ?? '') }}" placeholder="{{__('Enter Og Description')}}" >
                    ...
                    <div class="form-group">
                        ...
                        @error('meta_data.og_image')
                            <div class="parsley-errors-list">{{ $message }}</div>
                        @enderror
                        @if ($exam->meta_data['og_image'] ?? '')
                          <img src="{{lms_storage($exam->meta_data['og_image'])}}" class="img-thumbnail mt-3" style="height: 120px;" alt="">
                        @endif
                    </div>
                </div>
            </div>
